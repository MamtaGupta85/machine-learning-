...
#remove outliers in amount
def remove_outliers(df):
  df_out = pd.DataFrame()
  for key, subdf in df_fraud.groupby('isFraud'):
    m = np.mean(subdf.amount)
    st = np.std(subdf.amount)
    reduced_df = subdf[(subdf.amount>(m-st))& (subdf.amount<(m+st))]
...
smote = SMOTE()
X = df_fraud2.drop(columns=['isFraud'], axis=1)
y= df_fraud2['isFraud']
...
rfc_fraud = RandomForestClassifier(n_estimators=num_trees)
results_adb = cross_val_score(rfc_fraud, x, y_smote, cv=kfold)
